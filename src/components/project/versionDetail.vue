<template>
  <div class="version-detail">
    <a-spin class="version-detail-spin" :spinning="loading">
            ...
            <a-dropdown :trigger="['click']" placement="bottomCenter" v-model="visibleTaskMenu">
              ...
              <a-menu @click="doVersion" class="field-right-menu" slot="overlay">
                <a-menu-item key="delete">
                  <a-icon type="delete" theme="twoTone" two-tone-color="red"></a-icon>
                  <font color="red">删除版本</font>
                </a-menu-item>
              </a-menu>
            </a-dropdown>
          </template>
      ...
      <div class="task-wrap">
        <a-row class="task-content" type="flex" align="stretch" justify="center">
          <a-col :span="16" :xs="24" :sm="14" :md="16" :lg="16">
            <vue-scroll :ops="scrollOps">
              <div class="task-title" :class="{'disabled': disableEdit}">
                ...
                <a-tooltip :mouseEnterDelay="0.5" v-if="!disableEdit">
                  ...
                  <div class="title-text" @click="editTitle" v-show="!showEditName">{{version.name}}</div>
                </a-tooltip>
                          ...
                          <a-tooltip :mouseEnterDelay="0.5" v-if="!disableEdit">
                            ...
                            <div class="field-flex">
                              <a class="muted name" style="margin: 0">
                                ...
                                <template v-else>{{version.plan_publish_time_format}}</template>
                              </a>
                            </div>
                          </a-tooltip>
                          <div class="field-flex" v-else>
                    ...
                    <div class="field">
                      <div class="field-left">
                        ...
                        <span class="field-name">
                          发布内容
                          <span
                            v-show="versionTaskSchedule !== false"
                          >· {{versionTaskDoneNum}}/{{versionTaskList.length}}</span>
                        </span>
                      </div>
          ...
          <a-col class="log-wrap" :span="8" :xs="24" :sm="10" :md="8" :lg="8">
            <div class="header">
    ...
    >
      <div>
        <p v-show="publishTask.selectTaskList.length">
          已选择
          <span class="text-warning">{{publishTask.selectTaskList.length}}</span>
          项
        </p>
