  ...
  <div class="task-search">
    <a-spin :spinning="loading">
      ...
      <div class="search-content">
        <a-form class="m-t-md" :form="form" @submit.prevent="handleSubmit">
          <a-form-item label="标题" :colon="false" :labelCol="{ span: 6 }" :wrapperCol="{ span: 18 }">
            <a-input
              placeholder="搜索项目内任务"
              v-decorator="[
                                            'title',
...
export default {
  ...
  methods: {
    ...
    getProjectMemberList() {
      getProjectMember({ projectCode: this.projectCode, pageSize: 300 }).then(
        (res) => {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          if (params.end_time && params.end_time.length) {
            params.endTime = JSON.stringify([
              moment(params.end_time[0]).format("YYYY-MM-DD"),
              moment(params.end_time[1]).format("YYYY-MM-DD"),
            ]);
          } else {
            params.endTime = undefined;
          }
          if (params.begin_time && params.begin_time.length) {
            params.beginTime = JSON.stringify([
              moment(params.begin_time[0]).format("YYYY-MM-DD"),
              moment(params.begin_time[1]).format("YYYY-MM-DD"),
            ]);
          } else {
            params.beginTime = undefined;
          }
          if (params.create_time && params.create_time.length) {
            params.createTime = JSON.stringify([
          ...
          if (params.done_time && params.done_time.length) {
            params.doneTime = JSON.stringify([
              moment(params.done_time[0]).format("YYYY-MM-DD"),
              moment(params.done_time[1]).format("YYYY-MM-DD"),
            ]);
          } else {
