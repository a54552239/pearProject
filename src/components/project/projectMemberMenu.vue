...
export default {
  ...
  methods: {
    init() {
      ...
      if (this.taskCode) {
        getTaskMembers({ taskCode: this.taskCode, pageSize: 300 }).then(
          (res) => {
    ...
    inviteTaskMember() {
      ...
      this.listTemp.forEach((code) => {
        const index = this.taskMemberList.findIndex(
          (item) => code == item.code
        );
        if (index === -1) {
    ...
    search: _.debounce(function () {
      ...
      if (!this.keyword) {
        this.projectMemberList = JSON.parse(
          JSON.stringify(this.projectMemberListCopy)
        );
      }
      ...
      this.searching = true;
      this.projectMemberList = this.projectMemberList.filter(
