<template>
  <a-tabs tabPosition="left" defaultActiveKey="1" :animated="false" v-model="tabKey">
        ...
        <div class="content-item">
          <div class="infos">
            ...
            <a-select size="large" v-model="project.private">
              <a-select-option :key="0">公开项目（所有人都可通过链接访问，仅项目成员可编辑）</a-select-option>
              <a-select-option :key="1">私有项目（仅项目成员可查看和编辑）</a-select-option>
            </a-select>
          </div>
        </div>
        <div class="content-item">
    ...
    <a-tab-pane key="4" forceRender>
      <span slot="tab">
        <a-icon type="deployment-unit" />任务流转
      </span>
      <div class="config-content">
                ...
                <a-list-item slot="renderItem" slot-scope="item">
                  <span slot="actions">
                    <a-tooltip placement="top" title="编辑">
                      <a class="muted m-r-sm" @click="doTaskWorkflow(item)">
            ...
            <div v-show="doTaskWorkflowView">
              <div class="header">
                <p>
                  <a-button
                    size="large"
                    class="middle-btn m-r-sm"
                    @click="saveTaskWorkflow(false)"
                  >取消</a-button>
                  <a-button
                    size="large"
                    class="middle-btn"
                    type="primary"
                    @click="saveTaskWorkflow(true)"
                    :disabled="!canSaveTaskWorkflow"
                  >保存</a-button>
                </p>
              </div>
              <div class="workflow-content">
                <template v-if="!loadingWorkflowRule">
                  ...
                  <div class="workflow-rule-item">
                    ...
                    <a-select
                      size="large"
                      v-model="currentTaskWorkflowRule.firstObj"
                      @change="(value)=>workflowRuleChange(value,'firstObj')"
                    >
                      <a-select-option
                        v-for="(taskStage, index) in taskStages"
                        :value="taskStage.code"
                        :key="taskStage.code"
                      >{{taskStage.name}}</a-select-option>
                    </a-select>
                  </div>
                  ...
                  <template v-if="currentTaskWorkflowRule.firstObj">
                    <div class="workflow-rule-item">
                      ...
                      <a-select
                        size="large"
                        v-model="currentTaskWorkflowRule.firstAction.action"
                        @change="(value)=>workflowRuleChange(value,'firstAction.action')"
                      >
                        <a-select-option
                          v-for="(rule, index) in taskWorkflowRuleActions"
                          :value="rule.id"
                          :key="rule.id"
                    ...
                    <template v-if="currentTaskWorkflowRule.firstAction.action != -1">
                      <template v-if="currentTaskWorkflowRule.firstAction.action == 3">
                        <div class="workflow-rule-item">
                          ...
                          <a-select
                            size="large"
                            v-model="currentTaskWorkflowRule.firstAction.value"
                      ...
                      <div class="workflow-rule-item">
                        ...
                        <a-select
                          size="large"
                          v-model="currentTaskWorkflowRule.firstResult.action"
                          @change="(value)=>workflowRuleChange(value,'firstResult.action')"
                        >
                          <a-select-option
                            v-for="(type, index) in taskWorkflowRuleTypes"
                            :value="type.id"
                            :key="type.id"
                      ...
                      <template v-if="currentTaskWorkflowRule.firstResult.action === 0">
                        <div class="workflow-rule-item">
                          ...
                          <a-select
                            size="large"
                            v-model="currentTaskWorkflowRule.firstResult.value"
                        ...
                        <template v-if="currentTaskWorkflowRule.firstResult.value">
                          <div class="workflow-rule-item">
                            ...
                            <a-select
                              size="large"
                              v-model="currentTaskWorkflowRule.lastResult.value"
                        ...
                        <template>
                          <div class="workflow-rule-item">
                            ...
                            <a-select
                              size="large"
                              v-model="currentTaskWorkflowRule.state.value"
