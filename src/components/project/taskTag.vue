  ...
  <div>
    ...
    >
      <a-tab-pane :key="taskTag.code" v-for="taskTag in taskTagList">
        <div slot="tab">
          <div class="tag-item">
            <a-badge status="success" :class="`badge-${taskTag.color}`" />
            {{taskTag.name}}
          </div>
        </div>
                    ...
                    <a-list-item-meta>
                      <div class="task-list" slot="title" :class="{'done': item.done}">
                        <div>
                          ...
                          <router-link
                            class="text-default task-content"
                            :to="`/project/space/task/${code}/detail/${item.code}`"
                          >
                            <a-tooltip placement="top" v-if="item.executor && item.executor.avatar">
                              <template slot="title">
                            ...
                            <span
                              class="muted flex m-r tag-name"
                              v-for="tag in item.tags"
                              :key="tag.code"
                            >
                              <a-badge status="success" :class="`badge-${tag.tag.color} m-r-xs`" />
                              {{tag.tag.name}}
                            </span>
                          </router-link>
                        </div>
                        <div
                          class="label task-time"
                          :class="showTimeLabel(item.end_time)"
                          v-if="item.end_time"
                        >{{showTaskTime(item.begin_time, item.end_time)}}</div>
                      </div>
                    </a-list-item-meta>
                  </a-list-item>
                </a-list>
