<template>
  <a-popover
    v-model="showNotice"
    overlayClassName="header-notice"
    trigger="click"
      ...
      <a-spin :spinning="loading">
        <a-tabs class="header-notice-content" :tabBarGutter="25">
          <a-tab-pane key="1">
            <span slot="tab">
              消息
              <span v-if="total && totalSum['message']">({{totalSum['message']}})</span>
            </span>
          ...
          <a-tab-pane key="2">
            <span slot="tab">
              通知
              <span v-if="total && totalSum['notice']">({{totalSum['notice']}})</span>
            </span>
...
export default {
  ...
  watch: {
    socketAction(val) {
      if (val.action === "notice") {
        ...
        this.init();
        const permission = showMsgNotification(val.title, val.msg, {
          icon: val.data.notify.avatar,
        });
        if (permission === false) {
  ...
  methods: {
    ...
    getTasks() {
      selfList({ page: this.task.page, pageSize: this.task.pageSize }).then(
        (res) => {
