      ...
      <a-layout id="layout" :class="layoutClass">
        <a-layout-header class="project-nav" :class="{'collapsed':collapsed}">
          ...
          <div class="right-menu pc-view">
            ...
            <header-select class="action action-organization" v-if="organizationList.length > 1"></header-select>
            <header-notice class="action action-notice"></header-notice>
            <header-avatar class="action action-avatar" :type="1"></header-avatar>
          </div>
          <div class="right-menu wap-menu">
            <div class="action action-more" @click.stop="wap_visible=!wap_visible">
              <a-avatar :src="$store.state.userInfo.avatar" />
            </div>
            <a-drawer
              class="right-drawer"
              title
            ...
            >
              <div>
                ...
                ></header-select>
                <header-avatar class="action action-avatar" :type="2" @finish="wap_visible=false"></header-avatar>
              </div>
              <div class="m-r-lg" :style="{padding: '1rem 0'}" v-if="config.WS_URI">
                <a-badge
                  title="当前在线"
                  :count="online"
                  showZero
        ...
        <a-layout style="padding-top: 65px;">
          <a-sider breakpoint="md" collapsedWidth="0" collapsible v-model="collapsed">
            <a-menu
              :theme="theme"
              v-for="menu in menus"
            ...
            <a-icon :type="collapsed ? 'right' : 'left'" slot="trigger" />
          </a-sider>
          <a-layout class="main-content" :class="!collapsed ? 'toleft':''">
...
export default {
  ...
  methods: {
    footerClick() {
      ...
      this.$websocket.send(
        JSON.stringify({ uid: this.$store.state.userInfo.id })
      );
    },
    checkLayout() {
      //更新左侧菜单选中状态
