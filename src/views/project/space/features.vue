    ...
    <a-row class="project-navigation" align="middle" justify="center">
      ...
      <a-col class="nav-body" :span="8" :xs="24" :sm="14" :md="8" :lg="8">
        <ul class="nav-wrapper nav nav-underscore">
          ...
          <li class data-app="works" @click="$router.push('/project/space/files/' + code)">文件</li>
          <li class data-app="build" @click="$router.push('/project/space/overview/' + code)">概览</li>
          <li class="actives" data-app="build" @click="init">版本</li>
        </ul>
      </a-col>
    </a-row>
    <wrapper-content :showHeader="false">
      <div class="content-wrapper">
        <div class="content-item features-content">
          <div class="actions">
            <a-dropdown placement="bottomLeft" :trigger="['click']" v-if="currentProjectFeatures">
              <a class="m-r text-default features-list-dropdown">
                <span
                  class="m-r-xs"
                  :style="{minWidth:'2rem',display: 'inline-block',textAlign: 'justify'}"
                >{{currentProjectFeatures.name}}</span>
                <a-icon type="down" />
              </a>
              <a-menu
              ...
              >
                <a-menu-item
                  :key="projectFeatures.code"
                  v-for="projectFeatures in projectFeaturesList"
                >
                  <div class="menu-item-content">
                    <div>
                      <a-radio :checked="currentProjectFeatures.code == projectFeatures.code"></a-radio>
                    ...
                    <div>
                      ...
                      <a class="muted">
                        <a-icon
                          type="delete"
                          class="m-l"
                          @click.stop="deleteFeatures(projectFeatures)"
                        ></a-icon>
                      </a>
                    </div>
                  </div>
          ...
          <a-spin :spinning="loading" v-if="currentProjectFeatures">
            <template v-if="versionTotal">
              ...
              >
                ...
                >
                  <a-col
                    v-for="(version,versionIndex) in versionItem"
                    :key="'v_'+versionIndex"
                    :span="8"
                    :xs="24"
                    :sm="12"
                  ...
                  <a-col :span="8" :xs="24" :sm="12" :md="8" :lg="6">
                    <a-card
          ...
          <a-row :gutter="16">
            ...
            <a-col :span="12">
              <a-date-picker
                ...
                format="YYYY年MM月DD日 HH:mm"
                style="width: 100%"
                placeholder="选择预计发布时间"
                v-decorator="['planPublishTime']"
              ></a-date-picker>
            </a-col>
          </a-row>
        </a-form-item>
  ...
  methods: {
    ...
    changeVersionStatus(e, versionType, version, index) {
      ...
      if (e.key == 3) {
        ...
        changeStatus({ versionCode: version.code, status: e.key }).then(
          (res) => {
            this.getProjectVersionList();
          }
        );
