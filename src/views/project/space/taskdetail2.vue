                                    ...
                                    <a-menu-item key="lock">
                                           <a-icon :type="task.private ? 'lock' : 'unlock'"/>
                                            ...
                                            <div class="menu-desc muted">
                                                <span v-if="task.private">所有参与者可见</span>
                        ...
                        <div class="content-left">
                            <vue-scroll :ops="scrollOps">
                                <div class="task-title" :class="{'disabled': task.deleted}">
                                    <a-input auto-focus @blur="doName" v-model="task.name" size="large"
                                             v-show="showEditName"/>
                                    <a-tooltip :mouseEnterDelay="0.5" v-if="!task.deleted">
                                        <template slot="title">
                                            <span>点击即可编辑</span>
                                        </template>
                                        <div class="title-text" @click="showEditName = true" v-show="!showEditName">
                                            {{task.name}}
                                        </div>
                                    </a-tooltip>
                                        ...
                                        <div class="list-item" :class="{'log-comment': log.is_comment}"
                                            ...
                                            <template v-if="!log.is_comment">
                                                ...
                                                <div class="log-item log-txt">
                                                    <div>{{log.member.name}} {{log.remark}}</div>
...
<script>
    ...
    import {formatTaskTime, relativelyTaskTime, relativelyTime} from "../../../assets/js/dateTime";
    ...
    export default {
        ...
        methods: {
            ...
            showTaskDesc() {
                ...
                this.initContent(task.description)
            },
            initContent(value) {
                if (value) {
            ...
            getChildTasks() {
                getTasks({pcode: this.code, pageSize: 100}).then((res) => {
            ...
            changeModalHeight() {
                ...
                $(".task-detail").css("width", width + "px");

                const modalHeight = $(window).height() - 150;
                const LogHeight = $(window).height() - 315;
                $(".content-left").css("height", modalHeight + "px");
