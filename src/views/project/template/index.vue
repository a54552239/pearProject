  ...
  <div class="project-template-index">
          ...
          <div class="other-info muted">
            <div class="info-item task" v-if="item.task_stages.length">
              ...
              <a-breadcrumb separator=">">
                <a-breadcrumb-item v-for="task in item.task_stages" :key="task.name">{{task.name}}</a-breadcrumb-item>
              </a-breadcrumb>
            </div>
          </div>
        </a-list-item>
    ...
    >
      <a-form @submit.prevent="handleSubmit" :form="form">
        ...
        <a-form-item>
          <a-textarea placeholder="模板说明" :rows="2" v-decorator="['description']" />
        </a-form-item>
        <a-form-item>
