        ...
        <a-card v-if="inviteLink" :title="`来自 ${inviteLink.member.name} 的邀请`">
          <div class="header">
            <span>
              ...
              <span
                v-if="inviteLink.invite_type == 'project'"
              >加入项目「{{inviteLink.sourceDetail.name}}」</span>
              <span
                v-if="inviteLink.invite_type == 'organization'"
              >加入组织「{{inviteLink.sourceDetail.name}}」</span>
            </span>
          </div>
