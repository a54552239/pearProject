<template>
  <div class="admin-menu">
    <wrapper-content>
      ...
      <Table :columns="columns" :dataSource="dataSource" :loading="loading" :pagination="false">
        ...
        <template slot="action" slot-scope="text,record,index">
          ...
          <Divider type="vertical" />
          <a @click="rowClick(record,'status')">
    ...
    <Modal
      ...
      :footer="null"
    >
      <Form :autoFormCreate="(form)=>{this.form = form}" @submit.prevent="handleSubmit">
        <a-row :gutter="24">
          <a-col :span="12">
            <FormItem
              :form="form"
            ...
            >
              ...
              >
                ...
                <a-select-option
                  ...
                  :key="node.node"
                >{{node.node}} - {{node.title}}</a-select-option>
              </a-select>
              <!-- <a-tree-select
                             showSearch
        ...
        <FormItem>
          <div class="action-btn" style="text-align: center">
            <Button
              ...
              class="middle-btn"
            >保存</Button>
            <Button class="middle-btn" size="large" @click="actionInfo.modalStatus = false">取消</Button>
          </div>
        </FormItem>
      </Form>
    </Modal>
  ...
  methods: {
    ...
    rowClick(record, action) {
      ...
      if (action == "add" || action == "edit" || action == "new") {
        ...
        status
          ? forbid(record.id, record.status)
          : resume(record.id, record.status);
      } else if (action == "del") {
    ...
    handleSubmit() {
      let app = this;
      app.form.validateFields((err, values) => {
        if (!err) {
          app.handleOk();
        }
      });
